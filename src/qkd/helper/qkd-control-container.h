...
public:
   ...
   * \endcode
   *
   * \return An iterator which refers to the first pair in the container.
   */
  ...
  /**
   * \brief Get the number of nodes in the container.
   * \return The number of nodes.
   */
  uint32_t GetN (void) const;

  /**
   * \brief Manually add an entry to the container consisting of a previously composed 
   * entry std::pair.
   * \param The QKD control object.
   ...
   * \brief Manually add an entry to the container consisting of a previously composed 
   * entry std::pair.
   * \param The pair of QKD control objects.
   * \param ipInterfacePair The pair of a pointer to Ipv4 object and interface index of the Ipv4Interface to add to the container.
   */
  void Add (std::pair<Ptr<QKDControl>, uint32_t> ipInterfacePair);

  /**
   * \brief Get the std::pair of an Ptr<QKDControl> and interface stored at the location 
   * specified by the index.
   * \param i The index of the container entry to retrieve.
   * \return The pair of QKD controls and the interface index.
   *   
   */
...
private:
  ...
  typedef std::vector<std::pair<Ptr<QKDControl>,uint32_t> > InterfaceVector; //!< Container for pairs of QKDControl smart pointer / Interface Index.
