...
UUID::UUID(const std::string& uuid)
{
    ...
    for (auto ch : uuid)
    {
        ...
        if (pack)
        {
            ...
            if ((ui1 > 15) || (ui2 > 15))
                NS_FATAL_ERROR("Invalid UUID string: " + uuid);
...
UUID UUID::Sequential()
{
    ...
    if (UuidCreateSequential(&uuid) != RPC_S_OK)
        NS_FATAL_ERROR("Cannot generate sequential UUID!");
...
UUID UUID::Random()
{
    ...
    if (UuidCreate(&uuid) != RPC_S_OK)
        NS_FATAL_ERROR("Cannot generate random UUID!");
...
inline void 
swap(UUID& uuid1, UUID& uuid2) noexcept
{
    uuid1.swap(uuid2);
}
