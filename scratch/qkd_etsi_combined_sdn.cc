...
int main (int argc, char *argv[])
{
    ...
    uint32_t maliciousApplications = 0;
    ...
    numberOfNodes = (numberOfQKDLinks + numberOfETSI004ApplicationLinks + numberOfETSI014ApplicationLinks + maliciousApplications)*2 + 4; //extra 2 for KMSs + 1 Control + 1 SDN
    ...
    std::string ipv4BaseQKDControlSDN = "10.1.1.0";
    Ipv4Address addressBaseQKDControlSDN;
    addressBaseQKDControlSDN.Set(ipv4BaseQKDControlSDN.c_str());
    ipv4.SetBase (addressBaseQKDControlSDN, "255.255.255.0");
    Ipv4InterfaceContainer i0i3 = ipv4.Assign (d0d3);

    std::string ipv4BaseKMSASDN = "10.2.1.0";
    Ipv4Address addressBaseKMSASDN;
    addressBaseKMSASDN.Set(ipv4BaseKMSASDN.c_str());
    ipv4.SetBase (addressBaseKMSASDN, "255.255.255.0");
    Ipv4InterfaceContainer i1i3 = ipv4.Assign (d1d3);

    std::string ipv4BaseKMSBSDN = "10.3.1.0";
    Ipv4Address addressBaseKMSBSDN;
    addressBaseKMSBSDN.Set(ipv4BaseKMSBSDN.c_str());
    ipv4.SetBase (addressBaseKMSBSDN, "255.255.255.0");
    Ipv4InterfaceContainer i2i3 = ipv4.Assign (d2d3);

    QAHelper.InstallSDN(n.Get(3), i0i3.GetAddress(1), 3060);

    kmsA->SetSDNControllerAddress( InetSocketAddress(i1i3.GetAddress(1), 3060) ); //kmsNode, SDNAddress, SDNPort
    kmsB->SetSDNControllerAddress( InetSocketAddress(i2i3.GetAddress(1), 3060) ); //kmsNode, SDNAddress, SDNPort 
 
    /*
    //testing internet connection with ping
    NS_LOG_INFO ("Create V4Ping Appliation");
    Ptr<V4Ping> app = CreateObject<V4Ping> ();
    app->SetAttribute ("Remote", Ipv4AddressValue (i2i3.GetAddress(1)));
    app->SetAttribute ("Verbose", BooleanValue (true) );
    n.Get(2)->AddApplication (app);
    app->SetStartTime (Seconds (qkdStartTime));
    app->SetStopTime (Seconds (qkdStopTime));
    ...
    */
    ...
    if(numberOfETSI014ApplicationLinks && systemId == systemID0) {
    ...
    {   
        int i=(a+numberOfQKDLinks+numberOfETSI004ApplicationLinks)*2+4;
        int j=i+1;
        ...
        randInt = randomNumber % numberOfKeyToFetchFromKMSOptions.size();
        ...
        appAlice->SetAttribute("AESLifetime", UintegerValue(aesLifetime)); 
        appAlice->SetAttribute("NumberOfKeyToFetchFromKMS", UintegerValue (numberOfKeyToFetchFromKMS));//Number of keys to obtain per request!
        appAlice->SetAttribute("AuthenticationType", UintegerValue (authenticationType)); //(0-unauthenticated, 1-VMAC, 2-MD5, 3-SHA1)
        appAlice->SetAttribute("EncryptionType", UintegerValue (encryptionType)); //(0-unencrypted, 1-OTP, 2-AES)
        appAlice->SetAttribute("WaitInsufficient", TimeValue (Seconds (appHoldTime)));
        
        Ptr<QKDApp014> appBob = CreateObject<QKDApp014> (); 
        appBob->SetAttribute("AESLifetime", UintegerValue(aesLifetime)); 
        appBob->SetAttribute("NumberOfKeyToFetchFromKMS", UintegerValue (numberOfKeyToFetchFromKMS));//Number of keys to obtain per request!
        appBob->SetAttribute("AuthenticationType", UintegerValue (authenticationType)); //(0-unauthenticated, 1-VMAC, 2-MD5, 3-SHA1)
        appBob->SetAttribute("EncryptionType", UintegerValue (encryptionType)); //(0-unencrypted, 1-OTP, 2-AES)
        appBob->SetAttribute("WaitInsufficient", TimeValue (Seconds (appHoldTime)));
    ...
    if(maliciousApplications)
    {
        if(systemId == systemID0) {
            std::cout << "\n*********\n*** MaliciousApplication ETSI 004 Configuration\n*********\n";
        }

        Config::SetDefault ("ns3::QKDApp004::MaliciousApplication", UintegerValue (1));
        Config::SetDefault ("ns3::QKDApp004::DoSAttackIntensity", TimeValue (Seconds (attackIntensity)));

        for(uint32_t a=0; a<maliciousApplications ;a++)
        {   

            int i=(a+numberOfQKDLinks+numberOfETSI004ApplicationLinks+numberOfETSI014ApplicationLinks)*2+4;
            int j=i+1;
            ...
            double startTimeTemp = appStartTime + m_random->GetValue (10, 20);

            LinkDetails* linkD = new LinkDetails;
            linkD->nodes = std::to_string(n.Get(i)->GetId()) + "-" + std::to_string(n.Get(j)->GetId());
            //linkD->nodes = srcNodeId + "-" + dstNodeId;
            linkD->title = "MALICIOUS ETSI 004 Connection: " + linkD->nodes; 
            linkD->type = 1;
            linkD->m_encryptionType = encryptionType;
            linkD->m_authenticationType = authenticationType;
            linkD->m_aesLifeTime = aesLifetime;
            ...
            appAlice->SetAttribute("EncryptionType", UintegerValue (encryptionType)); //(0-unencrypted, 1-OTP, 2-AES)
            appAlice->SetAttribute("AESLifetime", UintegerValue (aesLifetime));
            appAlice->SetAttribute("MaliciousApplication", UintegerValue (1));
            appAlice->SetAttribute("DoSAttackIntensity", TimeValue (Seconds (attackIntensity)));
            appAlice->SetAttribute("Priority", UintegerValue (priority));
            appAlice->SetAttribute("TTL", UintegerValue (ttl));
            ...
            appBob->SetAttribute("EncryptionType", UintegerValue (encryptionType)); //(0-unencrypted, 1-OTP, 2-AES)
            appBob->SetAttribute("AESLifetime", UintegerValue (aesLifetime));
            appBob->SetAttribute("MaliciousApplication", UintegerValue (1));
            appBob->SetAttribute("DoSAttackIntensity", TimeValue (Seconds (attackIntensity)));
            appBob->SetAttribute("Priority", UintegerValue (priority));
            appBob->SetAttribute("TTL", UintegerValue (ttl));
    ...
    if(trace){
        ...
        p2p.EnableAsciiAll (ascii.CreateFileStream ("qkd_etsi_combined_sdn.tr"));
        p2p.EnablePcapAll ("qkd_etsi_combined_sdn");  
        AnimationInterface anim ("qkd_etsi_combined_sdn.xml");  // where "animation.xml" is any arbitrary filename
    }
