...
int main (int argc, char *argv[])
{
    ...
    numberOfNodes = (numberOfQKDLinks + numberOfETSI014ApplicationLinks)*2 + 4; //extra 2 for KMSs + 1 Control
    ...
    std::vector<uint32_t> AuthenticationTypes = {0,1,2}; //{0,1,2};
    std::vector<uint32_t> EncryptionTypes = {1,2};// {1,2}; //0,1,2};
    ...
    {   
        ...
        if(systemId == systemID0) {
            ...
            if(encryptionType == 0)
                std::cout << "EncryptionType: " << "Unencrypted" << std::endl;
            else if (encryptionType == 1)
                std::cout << "EncryptionType: " << "OTP" << std::endl;
            else if (encryptionType == 2)
                std::cout << "EncryptionType: " << "AES256" << std::endl;

            if(authenticationType == 0)
                std::cout << "AuthenticationType: " << "Unauthenticated" << std::endl;
            else if (authenticationType == 1)
                std::cout << "AuthenticationType: " << "VMAC" << std::endl;
            else if (authenticationType == 2)
                std::cout << "AuthenticationType: " << "MD5" << std::endl;
        ...
        std::cout <<"Connections.size():" << connectionIDs.size() << std::endl;;
        std::cout <<"Connection[0]:" << connectionIDs[0] << std::endl;; 
        std::cout <<"Connection[1]:" << connectionIDs[1] << std::endl;; 
    ...
    if(trace){
        ...
        p2p.EnableAsciiAll (ascii.CreateFileStream ("qkd_etsi_014.tr"));
        p2p.EnablePcapAll ("qkd_etsi_014");  
        AnimationInterface anim ("qkd_etsi_014.xml");  // where "animation.xml" is any arbitrary filename
    }
